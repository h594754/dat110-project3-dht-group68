...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		boolean keyMember = Util.checkInterval(key, node.getNodeID().add(BigInteger.ONE), stub.getNodeID());
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		List<NodeInterface> fintab = node.getFingerTable();
		// starting from the last entry, iterate over the finger table
		for(int i = fintab.size() - 1; i >= 0; i--) {
			NodeInterface fin = fintab.get(i);
			// for each finger, obtain a stub from the registry
			NodeInterface stub = Util.getProcessStub(fin.getNodeName(), fin.getPort());
			// check that finger is a member of the set {nodeID+1,...,ID-1} i.e. (nodeID+1 <= finger <= key-1) using the ComputeLogic
			
			assert stub != null;
			boolean closeFin = Util.checkInterval(fin.getNodeID(), node.getNodeID().add(BigInteger.ONE), ID.subtract(BigInteger.ONE));
